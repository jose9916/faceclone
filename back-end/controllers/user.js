...
exports.register = async (req, res) => {
  try {
    ...
    if (check) {
      return res.status(400).json({
        message:
          "The email address already exists, try width a different email address",
      });
    }
...
exports.sendResetPasswordCode = async (req, res) => {
  try {
    ...
    console.error("sendResetPasswordCode error:", error);
    res.status(500).json({ message: error.message });
  }
};
  ...
  try {
    if (req.user.id !== req.params.id) {
      ...
      if (
        ...
        // Quitar al sender de los followers del receiver
        await receiver.updateOne({
          $pull: { followers: sender._id },
        });

        // Quitar al receiver de los following del sender
        await sender.updateOne({
...
exports.acceptRequest = async (req, res) => {
  try {
    if (req.user.id !== req.params.id) {
      const receiver = await User.findById(req.user.id); // quien acepta
      const sender = await User.findById(req.params.id); // quien envió la solicitud
      ...
      return res.status(400).json({ message: "You can't accept yourself" });
    }
  } catch (error) {
    res.status(500).json({ message: error.message });
  }
...
exports.deleteRequest = async (req, res) => {
  try {
    if (req.user.id !== req.params.id) {
      const receiver = await User.findById(req.user.id); // quien recibe la solicitud
      const sender = await User.findById(req.params.id); // quien envió la solicitud
...
exports.getFriendsPageInfos = async (req, res) => {
  try {
    ...
    console.error("❌ getFriendsPageInfos error:", error);
    res.status(500).json({ message: error.message });
  }
};
