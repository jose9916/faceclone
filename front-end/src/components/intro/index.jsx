...
export function Intro({ detailss, visitor, setOtherName }) {
  ...
  const updateDetails = () => {
    console.log("sent");
    fetch(`${import.meta.env.VITE_APP_API}/updateDetails`, {
      method: "PUT",
      headers: {
      ...
      .then((res) => {
        if (!res.ok) {
          return res.json().then((err) => {
            throw new Error(err.message || "Error al actualizar los detalles");
          });
        }
        return res.json();
      })
      .then((data) => {
        setShowBio(false);
        setDetails(data); // ✅ el backend devuelve updated.details
        console.log(data);
        setOtherName(data.otherName);
      })
      .catch((error) => {
        console.error("Error al actualizar detalles:", error.message);
      });
  };
