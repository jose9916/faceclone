...
export function Cover({ cover, visitor, photos }) {
  ...
  const updateCoverPicture = async () => {
    try {
      ...
      if (updated_picture === "ok") {
        ...
        if (new_post.ok) {
          // ✅ Todo bien → refrescamos la página
          setLoading(false);
          setCoverPicture("");
          window.location.reload(); // 👈 refresca la página completa
        } else {
          setLoading(false);
          setError(new_post.message || "No se pudo crear el post.");
        }
      } else {
        setLoading(false);
        setError(
          typeof updated_picture === "string"
            ? updated_picture
